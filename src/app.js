...
const elements = {
  ...
  txtHudDirection: document.getElementById('hud-direction-label'),
  ...
  viewportArea: document.getElementById('viewport-area')
};
...
// Direction Pad Button Clicks (on canvas HUD)
elements.dirButtons.forEach(btn => {
...
elements.resizerLeft.addEventListener('mousedown', (e) => {
  isResizingLeft = true;
  startX = e.clientX;
  startLeftWidth = elements.sidebarLeft.offsetWidth;
  document.body.style.cursor = 'col-resize';
  document.body.style.userSelect = 'none';
});

elements.resizerRight.addEventListener('mousedown', (e) => {
  isResizingRight = true;
  startX = e.clientX;
  startRightWidth = elements.sidebarRight.offsetWidth;
  document.body.style.cursor = 'col-resize';
  document.body.style.userSelect = 'none';
});
...
document.addEventListener('mousemove', (e) => {
  if (isResizingLeft) {
    ...
    const newWidth = Math.max(280, Math.min(500, startLeftWidth + dx));
    ...
    const dx = e.clientX - startX;
    const newWidth = Math.max(280, Math.min(500, startRightWidth - dx));
    elements.sidebarRight.style.width = newWidth + 'px';
...
// --- CSS Variable initialization for sidebar widths ---
document.documentElement.style.setProperty('--sidebar-left-width', '360px');
document.documentElement.style.setProperty('--sidebar-right-width', '340px');
